...
#define dz_arrfree(a) ((!a) ? (void)0 : dz_arr_free_impl(dz_array_header(a)))
...
#define dz_arrpush(a, item)                                                    \
  do {                                                                         \
    if (!a) {                                                                  \
      ...
      dz_arr_maybe_grow(dz_array_header(a), sizeof(*a), (void **)&a);          \
    }                                                                          \
...
#define dz_arrinsert(a, index, item)                                           \
  do {                                                                         \
    dz_assert(a);                                                              \
    dz_arr_shift_at_index_impl(dz_array_header(a), index, sizeof(*a),          \
                               (void **)&a);                                   \
    a[index] = item;                                                           \
  } while (0);
...
extern void dz_arr_maybe_grow(DZArrayHeader *header, size_t element_size,
                              void **arr_ptr);
extern void dz_arr_maybe_shrink(DZArrayHeader *header, size_t element_size,
                                void **arr_ptr);
extern void dz_arr_free_impl(DZArrayHeader *arr);
extern void dz_arr_remove_impl(DZArrayHeader *header, size_t index_to_remove,
                               size_t element_size, void **arr_ptr);
extern void dz_arr_shift_at_index_impl(DZArrayHeader *header,
                                       size_t index_to_add, size_t element_size,
                                       void **arr_ptr);
